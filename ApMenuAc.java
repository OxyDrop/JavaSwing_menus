...
public class ApMenuAc extends JApplet implements ActionListener{
	...
	public void init() {
		...
		actions = new ActionColor[nbCouleurs];
		pan = new Panel();
		pan.setBackground(Color.cyan);
		//AJOUTS
		for(int i = 0; i<nbCouleurs;i++){
			actions[i] = new ActionColor(nomCouleurs[i],couleurs[i],nomIcones[i],pan);
...
class ActionColor extends AbstractAction{
	...
	private Panel pan;
	static ActionColor actionInactive; //Action inactive courante
	public ActionColor(String nom, Color couleur, String nomIcone, Panel pan){
